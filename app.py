    ...
    if st.button("取得最新消息與 AI 總結"):
        with st.spinner("抓取 Google 財經新聞中..."):
            ...
            if news_data:
                ...
                if api_key:
                    with st.spinner("🤖 AI 正在閱讀上述新聞並提煉重點..."):
                        try:
                            genai.configure(api_key=api_key)
                            model = genai.GenerativeModel('gemini-2.5-flash')
                            res = model.generate_content(f"請總結以下新聞的 3 個產業重點，用繁體中文條列式回答：\n\n{news_text}")
                            st.info("### 🤖 AI 重點總結")
                            st.write(res.text)
                        except Exception as e: 
                            st.error(f"❌ AI 錯誤：{e}")
                else: 
                    st.warning("⚠️ 請先在左側欄位輸入 Gemini API Key 來啟動自動總結功能！")
            else: 
                st.error("❌ 抓取失敗，目前沒有相關新聞。")